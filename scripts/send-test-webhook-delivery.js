...
async function main() {
  ...
  if (!webhook) throw new Error("wh_builtin_receiver missing");
  if (webhook.status !== "active") throw new Error("wh_builtin_receiver is not active");
  ...
  const payload = {
    ...
    data: {
      message: "GoodAppBackEnd webhook delivery log test",
  ...
  const headers = {
    ...
    "X-GoodOS-Signature": signature(webhook.secret, rawBody)
  };
