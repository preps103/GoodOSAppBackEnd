...
async function main(){
  ...
  for(const policy of policies.rows){
    ...
    try{
      const started=await client.query(`INSERT INTO goodbase_telemetry_retention_runs(organization_id,project_id,environment_id,status,evidence_json) VALUES($1,$2,$3,'running',jsonb_build_object('policyId',$4::text,'startedBy','scheduled-retention')) RETURNING id`,[policy.organization_id,policy.project_id,policy.environment_id,policy.id]);runId=started.rows[0].id;
      ...
      if(hold.rows[0]){await client.query(`UPDATE goodbase_telemetry_retention_runs SET status='blocked',evidence_json=evidence_json||jsonb_build_object('legalHoldId',$2::text),completed_at=NOW() WHERE id=$1`,[runId,hold.rows[0].id]);blocked++;continue;}
