...
router.post("/sdks/releases", mfaRequired, async (request, response, next) => {
  try {
    ...
    if(!version||!packageRef||!/^[0-9a-f]{40}$/.test(sourceCommit))return response.status(400).json({success:false,message:"Version, package reference, and an exact 40-character source commit are required."});
    ...
    const checksum=clean(request.body?.checksumSha256,64).toLowerCase();const signatureRef=clean(request.body?.signatureRef,500);const sbomRef=clean(request.body?.sbomRef,500);const changelogRef=clean(request.body?.changelogRef,500);
    if(!/^[0-9a-f]{64}$/.test(checksum)||!signatureRef||!sbomRef||!changelogRef)return response.status(400).json({success:false,message:"SDK release requires checksum, signature, SBOM, and changelog references."});
    const releaseId=`sdk_${sdk}_${version.replace(/[^a-zA-Z0-9]+/g,"_")}`;
    const result = await database.query(
      `INSERT INTO goodbase_sdk_releases(id,language,version,status,minimum_platform_version,artifact_url,package_ref,source_commit,capabilities_json,checksum_sha256,signed,test_report_json,signature_ref,sbom_ref,changelog_ref,compatibility_matrix_json)
       VALUES($1,$2,$3,'preview',$4,$5,$5,$6,$7::jsonb,$8,$9,$10::jsonb,$11,$12,$13,$14::jsonb) RETURNING *`,
      [releaseId,sdk,version,clean(request.body?.minimumPlatformVersion,80)||"1.0.0",packageRef,sourceCommit,
        JSON.stringify(capabilities),checksum,request.body?.signed===true,JSON.stringify(request.body?.testReport||{}),signatureRef,sbomRef,changelogRef,JSON.stringify(request.body?.compatibilityMatrix||{})]
    );
...
router.post("/sdks/releases/:id/conformance",mfaRequired,async(request,response,next)=>{try{const release=await database.query(`SELECT * FROM goodbase_sdk_releases WHERE id=$1 AND status='preview'`,[request.params.id]);if(!release.rows[0])return response.status(404).json({success:false,message:"Preview SDK release not found."});const commit=release.rows[0].source_commit;const ci=await database.query(`SELECT 1 FROM goodbase_release_evidence WHERE evidence_type='ci' AND release_commit=$1 AND status='passed' LIMIT 1`,[commit]);if(!ci.rows[0])return response.status(409).json({success:false,message:"Exact-commit CI evidence is required before recording SDK conformance."});const results=request.body?.results&&typeof request.body.results==='object'?request.body.results:{};const required=["rest","graphql","realtime","offline","storage","auth","attestation","product"];if(required.some((key)=>results[key]!=="passed"))return response.status(400).json({success:false,message:"Every SDK conformance family must pass."});const platform=clean(request.body?.clientPlatform,30);const artifactRef=clean(request.body?.artifactRef,500);if(!artifactRef)return response.status(400).json({success:false,message:"A retained conformance artifact is required."});const scenario=request.body?.scenarioReport&&typeof request.body.scenarioReport==='object'?request.body.scenarioReport:{};const result=await database.query(`INSERT INTO goodbase_sdk_compatibility_runs(release_id,platform_version,local_stack_version,status,results_json,client_platform,artifact_ref,device_model,os_version,encrypted_cache_verified,scenario_report_json,started_at,completed_at) VALUES($1,$2,$3,'passed',$4::jsonb,$5,$6,$7,$8,$9,$10::jsonb,NOW(),NOW()) RETURNING *`,[request.params.id,clean(request.body?.platformVersion,80),clean(request.body?.localStackVersion,80),JSON.stringify(results),platform||null,artifactRef,clean(request.body?.deviceModel,120)||null,clean(request.body?.osVersion,120)||null,request.body?.encryptedCacheVerified===true,JSON.stringify(scenario)]);await audit(request,"goodbase.sdk.conformance.record","sdk_compatibility_run",result.rows[0].id,{releaseId:request.params.id,clientPlatform:platform});return response.status(201).json({success:true,run:result.rows[0]});}catch(error){return next(error);}});

router.post("/sdks/releases/:id/activate",mfaRequired,async(request,response,next)=>{try{const conformance=await database.query(`SELECT COUNT(*)::int AS passed FROM goodbase_sdk_compatibility_runs WHERE release_id=$1 AND status='passed' AND results_json @> '{"rest":"passed","graphql":"passed","realtime":"passed","offline":"passed","storage":"passed","auth":"passed","attestation":"passed","product":"passed"}'::jsonb AND artifact_ref IS NOT NULL`,[request.params.id]);if(conformance.rows[0].passed<1)return response.status(409).json({success:false,message:"A retained full conformance run is required before SDK activation."});const result=await database.query(`UPDATE goodbase_sdk_releases SET status='active',released_at=NOW() WHERE id=$1 AND status='preview' AND signed=TRUE AND source_commit ~ '^[0-9a-f]{40}$' AND checksum_sha256 ~ '^[0-9a-f]{64}$' AND signature_ref IS NOT NULL AND sbom_ref IS NOT NULL AND changelog_ref IS NOT NULL AND compatibility_matrix_json<>'{}'::jsonb RETURNING *`,[request.params.id]);if(!result.rows[0])return response.status(409).json({success:false,message:"SDK release evidence is incomplete."});await audit(request,"goodbase.sdk.release.activate","sdk_release",request.params.id);return response.json({success:true,release:result.rows[0]});}catch(error){return next(error);}});
