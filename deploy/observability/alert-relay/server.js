...
http.createServer((request,response)=>{
  ...
  let body="",size=0;request.on("data",chunk=>{size+=chunk.length;if(size>1024*1024){request.destroy();return;}body+=chunk;});request.on("end",async()=>{stats.received+=1;try{JSON.parse(body);const status=await deliver(body);response.writeHead(202,{"content-type":"application/json"});response.end(JSON.stringify({accepted:true,receiverStatus:status}));}catch(error){response.writeHead(502,{"content-type":"application/json"});response.end(JSON.stringify({accepted:false,message:String(error.message).slice(0,200)}));}});
}).listen(port,"0.0.0.0");
